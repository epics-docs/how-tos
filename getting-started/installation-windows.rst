...
MSYS2 can be installed, used and updated without Administrator rights.
...
For GNU Make, the easiest way is to use the one included in Strawberry Perl. Otherwise, the Windows binary that is provided on the EPICS web site at https://epics.anl.gov/download/tools/make-4.2.1-win64.zip can be dropped anywhere in the `%PATH%`.
...
*Note:* When using the Visual Studio compilers, compilation uses different flags for building DLLs and building static libraries. You can't generate static and shared libraries in the same build and have to decide between static or DLL build globally for your installation.
...
If you cannot avoid paths with such characters, use the Windows short path (can be displayed with ``dir /x``) for all path components with those characters in any path settings and/or the directory you ``cd`` into for the build.
